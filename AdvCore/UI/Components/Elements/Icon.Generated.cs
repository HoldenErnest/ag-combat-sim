...
using System.Linq;
namespace AdvCore.UI.Components.Elements;
partial class Icon : global::Gum.Forms.Controls.FrameworkElement
{
    ...
    ColorCategory? _colorCategoryState;
    public ColorCategory? ColorCategoryState
    {
        ...
        set
        {
            ...
            if(value != null)
            {
                if(Visual.Categories.ContainsKey("ColorCategory"))
                {
                    var category = Visual.Categories["ColorCategory"];
                    var state = category.States.Find(item => item.Name == value.ToString());
                    this.Visual.ApplyState(state);
                }
                else
                {
                    var category = ((global::Gum.DataTypes.ElementSave)this.Visual.Tag).Categories.FirstOrDefault(item => item.Name == "ColorCategory");
                    var state = category.States.Find(item => item.Name == value.ToString());
                    this.Visual.ApplyState(state);
                }
            }
